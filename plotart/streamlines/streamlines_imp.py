...
def compute_trace(
    ...
    while (
        ...
        and n <= maxiter
        and sink_soak
        <= max_iter_sink_proximity  # allow only 20 more steps if getting close to sink
    ):
        ps.append(p.copy())
        dx = u_i(p[0], p[1])[0] * dt
        dy = v_i(p[0], p[1])[0] * dt
        ...
        for item in seeds:
            if item == "random":
                ...
                for p in seeds_center:
                    ...
                    self.streamtraces_color.extend(
                        (n_streamstraces_per_source + 1)
                ...
                self.streamtraces_color.extend(
                    np.random.choice(self.colors, n_streamtraces)
                )
